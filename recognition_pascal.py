...
from lib.data import Pascal_Evaluator, imagenet_tester
...
pascal_classes = ['aeroplane', 'bycicle', 'bird', 'boat', 'bottle', 'bus',
...
def main():
    ...
    if args.fraction:
        init_seed = np.random.seed(44)
        keys = [*dataset]
        proportion = int(round(len(keys)*args.fraction))
        interval = np.random.permutation(proportion)
        new_dataset = {}
        for idx  in interval:
            new_dataset[keys[idx]] = dataset[keys[idx]]

        dataset = new_dataset

    # ====================================================================
    experimental_dataset = Pascal_Evaluator(args.root_data, dataset,
                                            transform)
