...
# In-package imports
from models.transformer import CLS_KV
...
class ConvNeXt(nn.Module):
    def __init__(self, block_setting: List[CNBlockConfig],
        ...
        # CLS-Token
        ones_cls = torch.ones((1, 1, lastconv_output_channels))
        self.cls_token = nn.Parameter(torch.normal(mean=ones_cls,
                                                   std=ones_cls),
                                                   requires_grad=True)

        # Before the classifier
        self.cls_stream = CLS_KV(dim=lastconv_output_channels,
                                 heads=1,
                                 dim_head=lastconv_output_channels,
                                 ch_scaling=lastconv_output_channels,
                                 project_out=False)

        #self.cls_stream.to_out = nn.LayerNorm(lastconv_output_channels)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.features(x)
        _, cls_tokens = self.cls_stream(x, cls_tokens)
        cls_tokens = cls_tokens.unsqueeze(dim=-1).unsqueeze(dim=-1)
...
dict_convnext_final={'convnext_tiny_final': convnext_tiny,
                     'convnext_small_final': convnext_small,
                     'convnext_base_final': convnext_base,
                     'convnext_large_final': convnext_large}
