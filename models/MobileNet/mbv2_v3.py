...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        # CLS-token
        ones_cls = torch.ones((1, 1, self.last_channel))
        self.cls_token = nn.Parameter(torch.normal(mean=ones_cls,
                                                   std=ones_cls),
                                                   requires_grad=True)

        self.cls_stream = CLS_KV(dim=self.last_channel, heads=1,
                                 ch_scaling=self.last_channel,
                                 dim_head=self.last_channel,
                                 project_out=True)
        self.cls_stream.to_out.append(nn.LayerNorm(self.last_channel))
