...
class EfficientNet(nn.Module):
    def __init__(
        ...
        for idx, cnf in enumerate(inverted_residual_setting):
            ...
            if idx+1 < len(inverted_residual_setting):
                cfg = inverted_residual_setting[idx+1]
        ...
        ca_blocks.append(CLS_Attention(lastconv_output_channels, heads=1,
                                       dim_head=lastconv_input_channels,
                                       project_out=True))
        # Before GAP
        ca_blocks.append(CLS_Attention(lastconv_output_channels, heads=1,
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        for mb_block, cls_block in zip(self.features, self.cls_stream):
            x = mb_block(x)
            attn, cls_tokens = cls_block(x, cls_tokens)
...
# ========================================================================
def efficientnet_b0(*, weights: efficientnet.EfficientNet_B0_Weights.IMAGENET1K_V1=None,
...
def efficientnet_b4(
    *, weights: efficientnet.EfficientNet_B4_Weights.IMAGENET1K_V1=None,
    progress: bool = True, **kwargs: Any) -> EfficientNet:
