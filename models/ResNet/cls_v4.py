...
# In package imports
from models.transformer import CLS_KV, CLS_FeedForward
...
# ========================================================================
def conclsv4x1(in_planes: int, out_planes: int, stride: int = 1) -> nn.Conv2d:
    """1x1 convolution"""
    ...
    def __init__(self, inplanes: int, planes: int, stride: int = 1,
        ...
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in"\
                                      " BasicBlock")
        # Both self.conclsv4 and self.downsample layers downsample the 
        # input when stride != 1
        self.override = False
        self.conclsv4 = conv3x3(inplanes, planes, stride)
...
class Bottleneck(nn.Module):
    ...
    # stride at the first 1x1 convolution(self.conclsv4) according to "Deep
    # residual learning for image recognition" 
    # https://arxiv.org/abs/1512.03385 
    # This variant is also known as ResNet V1.5 and improves accuracy
    # according to
    # https://ngc.nvidia.com/catalog/model-scripts/nvidia:resnet_50_clsv4_5_for_pytorch.
    ...
    def __init__(self, inplanes: int, planes: int, stride: int = 1,
        ...
        # input when stride != 1
        self.override = False
        self.conclsv4 = conclsv4x1(inplanes, width)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.conv3 = conclsv4x1(width, planes * self.expansion)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block: Type[Union[BasicBlock, Bottleneck]],
                 ...
                 norm_layer: Optional[Callable[..., nn.Module]] = None,
                 mixed: bool = False, project: bool = False,
                 fixed_dim: bool = False, leaky: bool = False
                 ) -> None:
        ...
        stream_depth = 512*block.expansion
        self.cls_token = nn.Parameter(torch.randn(1, 1, stream_depth))
        ...
        self.base = self._make_fixed(stream_depth,
                                     scaling=block.expansion*64)
        self.l1 = self._make_fixed(stream_depth,
                                   scaling=block.expansion*64)
        self.l2 = self._make_fixed(stream_depth,
                                   scaling=block.expansion*128)
        self.l3 = self._make_fixed(stream_depth,
                                   scaling=block.expansion*256)
        self.l4 = self._make_fixed(stream_depth,
                                   scaling=block.expansion*512)
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]],
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conclsv4x1(self.inplanes, planes * block.expansion, stride),
    ...
    def _make_fixed(self, depth: int=512, scaling: int=64):
        layer = CLS_KV(depth, heads=1, dim_head=depth,
                     project_out=self.project, ch_scaling=scaling)
        return layer

    def _make_increasing(self, depth: int=512):
        layer = CLS_Attention(depth, heads=1,
                              dim_head=depth, project_out=self.project)
        return layer

    def _increasing_layers(self, block):
        cls_ones = torch.ones((1, 1, block.expansion*64))
        self.cls_token = nn.Parameter(torch.normal(mean=cls_ones,
                                                   std=cls_ones))
        self.base = self._make_increasing(depth=block.expansion*64,
                                          dim_head=64,
                                          leaky=self.leaky)
        self.ffb = CLS_FeedForward(64*block.expansion, 128*block.expansion)

        self.l1 = self._make_increasing(depth=block.expansion*128,
                                        dim_head=block.expansion*64,
                                        leaky=self.leaky)
        self.ff1 = CLS_FeedForward(128*block.expansion, 256*block.expansion)
...
#        self.l2
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        cls_tokens = self.base[0](x, cls_tokens)
        cls_tokens = self.base[1](cls_tokens)

        x = self.layer1(x)
        cls_tokens = self.l1[0](x, cls_tokens)
        cls_tokens = self.l1[1](cls_tokens)

        x = self.layer2(x)
        cls_tokens = self.l2[0](x, cls_tokens)
        cls_tokens = self.l2[1](cls_tokens)

        x = self.layer3(x)
        cls_tokens = self.l3[0](x, cls_tokens)
        cls_tokens = self.l3[1](cls_tokens)


        x = self.layer4(x)
        cls_tokens = self.l4[0](x, cls_tokens)
        cls_tokens = self.l4[1](cls_tokens)
...
# ========================================================================
dict_clsv4 ={'resnet18_clsv4': resnet18,
              'resnet34_clsv4': resnet34,
              'resnet50_clsv4': resnet50,
              'resnet101_clsv4': resnet101,
              'resnet152_clsv4': resnet152,
              'resnext50_32x4d_clsv4': resnet.resnext50_32x4d,
              'resnext101_32x8d_clsv4': resnext101_32x8d,
              'wide_resnet50_2_clsv4': wide_resnet50_2,
              'wide_resnet101_2_clsv4': wide_resnet101_2}
