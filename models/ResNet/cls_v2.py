...
# In package imports
from models.transformer import CLS_Attention, CLS_FeedForward
...
class ResNet(nn.Module):
    ...
    def __init__(self, block: Type[Union[BasicBlock, Bottleneck]],
                 ...
                 dropout: bool = False, heads: bool = False,
                 shared: bool = False
                 ) -> None:
    ...
    def _make_increasing(self, depth: int=512, dim_head: int=256):
        layer = CLS_Attention(depth, heads=1, dim_head=dim_head, 
                              project_out=True)
        return layer
    ...
    def _increasing_layers(self, block):
        ...
        #self.ff2 = CLS_FeedForward(256*block.expansion, 512*block.expansion)

        self.l3 = self._make_increasing(depth=block.expansion*512,
                                        dim_head=block.expansion*256)
        self.ff3 = CLS_FeedForward(512*block.expansion, 1024*block.expansion)

        self.l4 = self._make_increasing(depth=block.expansion*512,
                                        dim_head=block.expansion*512)
        
        self.ff4 = CLS_FeedForward(512*block.expansion, 1024*block.expansion)

    def _fixed_layers(self, block):
        stream_depth = 512*block.expansion
        ones_cls = torch.ones((1, 1, stream_depth))
        self.cls_token = nn.Parameter(torch.normal(mean=ones_cls,
                                                   std=ones_cls))

        self.base = self._make_fixed(stream_depth, scaling=block.expansion*64)
        self.ffb = CLS_FeedForward(stream_depth, 256*block.expansion)

        self.l1 = self._make_fixed(stream_depth, scaling=block.expansion*64)
        self.ff1 = CLS_FeedForward(stream_depth, 256*block.expansion)      

        self.l2 = self._make_fixed(stream_depth, scaling=block.expansion*128)
        self.ff2 = CLS_FeedForward(stream_depth, 256*block.expansion)

        self.l3 = self._make_fixed(stream_depth, scaling=block.expansion*256)
        self.ff3 = CLS_FeedForward(stream_depth, 256*block.expansion)

        self.l4 = self._make_fixed(stream_depth, scaling=block.expansion*512)
        self.ff4 = CLS_FeedForward(stream_depth, 256*block.expansion)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        #_, cls_tokens = self.base(x, cls_tokens)
        #cls_tokens = self.ffb(cls_tokens)

        x = self.layer1(x)
        #_, cls_tokens = self.l1(x, cls_tokens)
        #cls_tokens = self.ff1(cls_tokens)

        x = self.layer2(x)
        #_, cls_tokens = self.l2(x, cls_tokens)
        #cls_tokens = self.ff2(cls_tokens)

        x = self.layer3(x)
        _, cls_tokens = self.l3(x, cls_tokens)
        cls_tokens = self.ff3(cls_tokens)

        x = self.layer4(x)
        _, cls_tokens = self.l4(x, cls_tokens)
        cls_tokens = self.ff4(cls_tokens)
...
# ========================================================================
dict_clsv2 ={'resnet18_clsv2': resnet18,
             'resnet34_clsv2': resnet34,
             'resnet50_clsv2': resnet50,
             'resnet101_clsv2': resnet101,
             'resnet152_clsv2': resnet152,
             'resnext50_32x4d_clsv2': resnet.resnext50_32x4d,
             'resnext101_32x8d_clsv2': resnext101_32x8d,
             'wide_resnet50_2_clsv2': wide_resnet50_2,
             'wide_resnet101_2_clsv2': wide_resnet101_2}
