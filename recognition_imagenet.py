...
def main():
    ...
    parser.add_argument('--pred', action='store_true', default=False,
                        help='Generation for groundtruth or predicted labels?')
    ...
    # Diagnostics
    #args.path_salient = osj('RebuttalDiags', args.state_root.replace(\
    #                        sstate, ''), '{}{}{}'.format(\
    #                        args.method, suffix, suf_alpha))
        ...
        for key in base_sdict:
            try:
                ...
                try:
                    ...
                    #print('Eror loading key {}'.format(key))
                    continue
    ...
    if 'hybrid' in args.method:
        target = hybrid_targetter(net, args)
    elif 'cam' in args.method:
